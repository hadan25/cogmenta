...
repo_root = os.path.abspath(parent_dir)  # Get the full repository root path

# Add necessary paths in correct order
sys.path.insert(0, repo_root)  # Add the full repo path first
sys.path.insert(0, current_dir)  # Then add the current directory

# Special handling for absolute_zero module
absolute_zero_path = os.path.join(repo_root, 'absolute_zero')
if os.path.exists(absolute_zero_path):
    sys.path.insert(0, absolute_zero_path)

# Make sure the absolute_zero module can be imported directly
print(f"Adding paths: current_dir={current_dir}, repo_root={repo_root}, absolute_zero_path={absolute_zero_path}")
...
try:
    ...
    print(f"Paths: sys.path={sys.path}")
...
save_dir = "C:\\Users\\ha25a\\.vscode\\cogmenta_core\\fixed_hybrid_results\\memory_20250517_162822\\memory\\cycle_1\\absolute"
...
try:
    if model_type == "memory":
        ...
        model = MemorySNN(neuron_count=1000)
        ...
        model = DecisionSNN(neuron_count=1000)
        ...
        model = MetacognitiveSNN(neuron_count=1000)
        ...
        model = PerceptualSNN(neuron_count=1000)
        ...
        model = ReasoningSNN(neuron_count=1000)
        ...
        model = AffectiveSNN(neuron_count=1000)
