...
class PrologEngine:
    ...
    def debug_dump_facts(self):
        """Print all facts in the KB for debugging"""
        results = list(self.prolog.query("confident_fact(P, S, O, C)"))
        print(f"[Prolog] Knowledge base contains {len(results)} facts:")
        for fact in results:
            print(f"  {fact['P']}({fact['S']}, {fact['O']}) [conf={fact['C']}]")
            
    def get_all_facts(self):
        """Retrieve all facts from the Prolog knowledge base
        
        Returns:
            list: List of (predicate, subject, object, confidence) tuples
        """
        facts = []
        try:
            query_results = list(self.prolog.query("confident_fact(P, S, O, C)"))
            ...
            print(f"[Prolog] Error retrieving facts: {e}")
        return facts
            
    def get_state_vector(self):
        """Get a simplified state vector representation of the knowledge base
        
        Returns:
            list: Vector representation of the KB state
        """
        # Simple implementation - count facts by predicate type
        facts = self.get_all_facts()
        
        # Create predicate counter
        pred_counter = {}
        for fact in facts:
            pred = fact[0]
            if pred not in pred_counter:
                pred_counter[pred] = 0
            pred_counter[pred] += 1
        
        # Create fixed length vector (up to 10 predicates)
        state_vector = [0] * 10
        for i, (pred, count) in enumerate(sorted(pred_counter.items())):
            if i < 10:
                state_vector[i] = min(1.0, count / 10.0)  # Normalize
