...
import time
# Add to abstraction_validation.py
import pymc as pm
import numpy as np
...
class BayesianValidator:
    ...
    def create_validation_model(self, concept_name):
        # Create a Bayesian model for validating a concept
        with pm.Model() as model:
            # Prior belief about validity
            validity = pm.Beta('validity', alpha=2, beta=2)
            
            # Likelihood of observing evidence given validity
            evidence = pm.Bernoulli('evidence', p=validity, observed=np.array([1, 1, 0, 1]))
            
            # Store the model
            self.models[concept_name] = model
            
    def validate_with_uncertainty(self, concept_name, new_evidence):
        if concept_name not in self.models:
            self.create_validation_model(concept_name)
            
        model = self.models[concept_name]
        with model:
            # Update with new evidence
            pm.set_data({'evidence': new_evidence})
            # Sample posterior
            trace = pm.sample(1000)
            
        # Return mean and credible interval for validity
        validity_samples = trace.posterior['validity'].values.flatten()
        return {
            'mean_validity': validity_samples.mean(),
            'credible_interval': (np.percentile(validity_samples, 2.5), 
                                 np.percentile(validity_samples, 97.5))
        }
