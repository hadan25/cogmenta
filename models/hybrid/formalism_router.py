...
class FormalismRouter:
    ...
    def __init__(self, prolog_engine=None, vector_engine=None, 
        ...
        # Initialize weights for different formalisms
        self.formalism_weights = {
            FormalismType.PROLOG: 1.0,
            FormalismType.VECTOR_SYMBOLIC: 1.0,
            FormalismType.PROBABILISTIC: 1.0,
            FormalismType.DESCRIPTION_LOGIC: 1.0,
            FormalismType.ASP: 1.0
        }
    ...
    def update_weight(self, formalism_type, weight_value):
        """
        Update the weight for a specific formalism type.
        
        Args:
            formalism_type (FormalismType): The formalism type to update
            weight_value (float): The new weight value
            
        Returns:
            bool: True if successful, False otherwise
        """
        if formalism_type not in self.formalism_weights:
            self.logger.warning(f"Cannot update weight for unknown formalism type: {formalism_type}")
            return False
            
        # Update the weight
        self.formalism_weights[formalism_type] = weight_value
        self.logger.info(f"Updated weight for {formalism_type.value} to {weight_value}")
        
        return True
        
    def get_weight(self, formalism_type):
        """
        Get the current weight for a specific formalism type.
        
        Args:
            formalism_type (FormalismType): The formalism type
            
        Returns:
            float: The current weight value or 1.0 if not found
        """
        return self.formalism_weights.get(formalism_type, 1.0)
    ...
    def determine_formalism(self, text):
        ...
        # Score each formalism based on domain patterns
        for formalism, patterns in self.domain_patterns.items():
            for pattern in patterns:
                if re.search(pattern, text_lower):
                    formalism_scores[formalism] += 1
        
        # Apply weights to scores
        for formalism in formalism_scores:
            formalism_scores[formalism] *= self.formalism_weights.get(formalism, 1.0)
        
        # Get formalism with highest score
        best_formalism = max(formalism_scores.items(), key=lambda x: x[1])
        
        # If no formalism scored any points, use a default based on input complexity
