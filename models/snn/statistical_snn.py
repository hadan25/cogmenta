...
class StatisticalSNN(EnhancedSpikingCore):
    """
    ...
    def __init__(self, neuron_count=1000, embedding_dim=300, learning_rate=0.01, topology_type="scale_free", vector_dim=None, bidirectional_processor=None):
        """
        Initialize the statistical generalization SNN.
        
        Args:
            neuron_count: Number of neurons in the network
            embedding_dim: Dimension for concept embeddings (legacy)
            learning_rate: Base learning rate for plasticity
            topology_type: Type of network topology
            vector_dim: Dimension of the vector space for token embeddings (standardized)
            bidirectional_processor: Optional BidirectionalProcessor instance
        """
        # Set embedding dimension for compatibility with both old and new interfaces
        self.embedding_dim = embedding_dim
        # Use vector_dim if provided, otherwise use embedding_dim
        vector_dim = vector_dim or embedding_dim
        ...
        # Set additional properties needed for statistical processing
        ...
        print(f"[StatSNN] Initialized Statistical SNN with {neuron_count} neurons, {vector_dim}D embeddings")
    ...
    def process_input(self, input_data, query_type='similarity'):
        ...
        """
        # Support both task_type (from DualSNNCoordinator) and query_type parameters
        if isinstance(query_type, str) and query_type in ['abduction', 'hypothesis', 'causal']:
            # Map these task types to similarity for compatibility
    ...
    def evaluate_concept_similarity(self, input_data, concept):
        """
        Evaluate similarity between input data and a concept.
        
        Args:
            input_data: Input data or spike pattern
            concept: Concept name to compare with
            
        Returns:
            Dictionary with similarity score and success flag
        """
        # Get embedding for input data
        input_embedding = self._get_input_embedding(input_data)
        
        # Get embedding for concept (if it exists)
        if concept in self.concept_embeddings:
            concept_embedding = self.concept_embeddings[concept]
            
            # Calculate cosine similarity
            similarity = np.dot(input_embedding, concept_embedding) / (
    ...
    def evaluate_relation(self, source, relation, target):
        """
        Evaluate a knowledge graph relation.
        ...
        Returns:
            Dictionary with similarity score and success flag
        """
        # Check if concepts exist in embeddings
        if source not in self.concept_embeddings or target not in self.concept_embeddings:
            return {
                'similarity': 0.0,
                'success': False
            }
        ...
        # Get embeddings
        source_embedding = self.concept_embeddings[source]
        target_embedding = self.concept_embeddings[target]
        
        # Simple relation evaluation (can be enhanced with actual relation modeling)
        # For now, just check if source and target are related
    ...
    def learn_relation(self, source, relation, target, learn_rate=0.01):
        """
        Learn a knowledge graph relation.
        ...
        Returns:
            Dictionary with error and success flag
        """
        # Initialize embeddings for concepts if they don't exist
        if source not in self.concept_embeddings:
            self.concept_embeddings[source] = np.random.normal(0, 0.1, self.embedding_dim)
            
        if target not in self.concept_embeddings:
            self.concept_embeddings[target] = np.random.normal(0, 0.1, self.embedding_dim)
            
        # Get embeddings
        source_embedding = self.concept_embeddings[source]
        target_embedding = self.concept_embeddings[target]
        
        # Simple relation learning: move embeddings closer together
        # This is a simplified approach; in a full implementation, we would model
        # relations as transformations in the embedding space
        
        # Calculate current similarity
        current_sim = np.dot(source_embedding, target_embedding) / (
            np.linalg.norm(source_embedding) * np.linalg.norm(target_embedding) + 1e-10
        )
        
        # Calculate error (1 - similarity)
        error = 1.0 - (current_sim + 1) / 2  # Normalize to [0, 1]
        
        # Update embeddings to be more similar
        # Move source embedding towards target
        source_update = learn_rate * error * (target_embedding - source_embedding)
        self.concept_embeddings[source] = source_embedding + source_update
        
        # Move target embedding towards source
        target_update = learn_rate * error * (source_embedding - target_embedding)
        self.concept_embeddings[target] = target_embedding + target_update
        
        # Normalize embeddings
        self.concept_embeddings[source] /= np.linalg.norm(self.concept_embeddings[source]) + 1e-10
        self.concept_embeddings[target] /= np.linalg.norm(self.concept_embeddings[target]) + 1e-10
        
        # Update concept clusters
        self._update_concept_clusters(source, self.concept_embeddings[source])
        self._update_concept_clusters(target, self.concept_embeddings[target])
    ...
    def train_concept(self, input_data, concept, learn_rate=0.01):
        """
        Train the model to associate input data with a concept.
        
        Args:
            input_data: Input data or spike pattern
            concept: Concept name to associate with
            learn_rate: Learning rate
        ...
        # Initialize concept embedding if it doesn't exist
        if concept not in self.concept_embeddings:
            self.concept_embeddings[concept] = input_embedding.copy()
            self._update_concept_clusters(concept, self.concept_embeddings[concept])
            return {
                'error': 0.0,
                'success': True
            }
        ...
        # Calculate current similarity
        current_sim = np.dot(input_embedding, concept_embedding) / (
            np.linalg.norm(input_embedding) * np.linalg.norm(concept_embedding) + 1e-10
        )
        
        # Calculate error (1 - similarity)
        error = 1.0 - (current_sim + 1) / 2  # Normalize to [0, 1]
        
        # Update concept embedding towards input
        update = learn_rate * error * (input_embedding - concept_embedding)
        self.concept_embeddings[concept] = concept_embedding + update
        
        # Normalize embedding
        self.concept_embeddings[concept] /= np.linalg.norm(self.concept_embeddings[concept]) + 1e-10
    ...
    def process_text_input(self, text_input, timesteps=20):
        ...
        Args:
            text_input: Text input for statistical processing
            timesteps: Number of timesteps for spike patterns
            
        Returns:
            Processed spike patterns for statistical processing
        """
        ...
        if hasattr(self, 'bidirectional_processor') and self.bidirectional_processor is not None:
            try:
                ...
                # Process with statistical analysis if needed
                return spike_patterns
            except Exception as e:
                print(f"[StatisticalSNN] Error processing text with bidirectional processor: {e}")
                # Fall back to legacy encoding
        
        # Legacy encoding if bidirectional processor is not available or error occurs
        # Extract statistical features from text
        token_features = self._extract_statistical_features(text_input)
        # Convert to activation pattern
        activation = self._encode_token_features(token_features)
        # Simulate spiking to get patterns
        spike_patterns = self._propagate_activation(timesteps)
        
        return spike_patterns
    
    def generate_text_output(self, spike_patterns, max_length=100):
        """
        Generate text output from statistical spike patterns using the standardized bidirectional processor.
        
        Args:
            spike_patterns: Spike patterns from statistical processing
            max_length: Maximum length of generated text
            
        Returns:
            Generated text with statistical information
        """
        # Check if we have bidirectional processor
        if hasattr(self, 'bidirectional_processor') and self.bidirectional_processor is not None:
            try:
                ...
                print(f"[StatisticalSNN] Error generating text with bidirectional processor: {e}")
                # Fall back to legacy approach
        
        # Legacy approach for text generation
        # Generate text about most activated regions
        region_activations = {}
        for region_name, region in self.regions.items():
            region_activations[region_name] = region.get('activation', 0.0)
        
        # Sort regions by activation
        sorted_regions = sorted(region_activations.items(), key=lambda x: x[1], reverse=True)
        
        # Generate text output
        output_text = "Statistical analysis: "
        
        if sorted_regions:
            top_regions = sorted_regions[:3]  # Top 3 most active regions
            region_texts = []
            
            for region_name, activation in top_regions:
                if activation > 0.1:  # Only mention significantly active regions
                    region_texts.append(f"{region_name} ({activation:.2f})")
            
            if region_texts:
                output_text += "Active regions: " + ", ".join(region_texts)
            else:
                output_text += "No significant statistical patterns detected"
        else:
            output_text += "No region activations available"
        
        return output_text
    
    def _extract_statistical_features(self, text_input):
        """
        Legacy method to extract statistical features from text.
        
        Args:
            text_input: Input text for statistical analysis
            
        Returns:
            Dictionary of token features
        """
        # Simple analysis of text statistics
        ...
        features = {
            ...
            'average_token_length': sum(len(word) for word in words) / max(1, len(words)),
            'token_frequencies': word_counts,
            'tokens': words
        }
        
        return features
    
    def _encode_token_features(self, features):
        """
        Legacy method to encode token features to activation pattern.
        
        Args:
            features: Token features dictionary
        ...
        input_neurons = []
        for region_name in ['embedding', 'similarity']:
            if region_name in self.regions:
                input_neurons.extend(self.regions[region_name]['neurons'])
        
        if not input_neurons:
            # Fallback to using a portion of all neurons
            input_neurons = list(range(min(self.neuron_count, 200)))
        
        # Encode basic features to activation values
        if features and input_neurons:
            # Normalize statistics to create activation patterns
            unique_ratio = features.get('unique_tokens', 0) / max(1, features.get('total_tokens', 1))
            avg_length = min(1.0, features.get('average_token_length', 0) / 10.0)  # Normalize to 0-1
            
            # Distribute activations across input neurons
            neurons_per_feature = max(1, len(input_neurons) // 4)
            
            # Activate neurons based on unique token ratio
            start_idx = 0
            end_idx = min(start_idx + neurons_per_feature, len(input_neurons))
            for i in range(start_idx, end_idx):
                neuron_idx = input_neurons[i]
                activation[neuron_idx] = unique_ratio * 0.8  # Scale activation
            
            # Activate neurons based on average token length
            start_idx = neurons_per_feature
            end_idx = min(start_idx + neurons_per_feature, len(input_neurons))
            for i in range(start_idx, end_idx):
                neuron_idx = input_neurons[i]
                activation[neuron_idx] = avg_length * 0.7  # Scale activation
            
            # Activate neurons for individual tokens (if space allows)
            tokens = features.get('tokens', [])
            if tokens and len(input_neurons) > 2 * neurons_per_feature:
                tokens_section = input_neurons[2 * neurons_per_feature:]
                # Map each token to a neuron or small neuron group
                neurons_per_token = max(1, len(tokens_section) // len(tokens))
                
                for i, token in enumerate(tokens[:len(tokens_section) // neurons_per_token]):
                    start_token_idx = 2 * neurons_per_feature + i * neurons_per_token
                    end_token_idx = min(start_token_idx + neurons_per_token, len(input_neurons))
                    
                    # Set activation based on token frequency
                    frequency = features.get('token_frequencies', {}).get(token, 1)
                    norm_freq = min(1.0, frequency / 5.0)  # Normalize frequency
                    
                    for j in range(start_token_idx, end_token_idx):
                        neuron_idx = input_neurons[j]
                        activation[neuron_idx] = 0.5 + (norm_freq * 0.5)  # Scale by frequency
