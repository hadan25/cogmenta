...
class ReflectionProcess:
    ...
    def reflect(self, components=None):
        ...
        # Add components info to reflection record if specified
        if components:
            reflection["focus_components"] = components
        
        # Analyze performance patterns
        patterns = self._analyze_performance_patterns()
        
        # Filter patterns by components if specified
        if components:
            patterns = [p for p in patterns if p.get("component") in components]
